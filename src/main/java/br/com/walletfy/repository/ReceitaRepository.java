...
public interface ReceitaRepository extends JpaRepository<Receita, Long>{
	...
	@Query(value = """
			...
			SELECT
			    ...
			    ROUND(
			        ((RR.RECEITAS - LAG(RR.RECEITAS) OVER (ORDER BY RR.MES)) / NULLIF(LAG(RR.RECEITAS) OVER (ORDER BY RR.MES), 0)) * 100,
			    1) AS VARIACAO_RECEITAS,
			    ROUND(
			        (((RR.RECEITAS - COALESCE(RG.GASTOS, 0)) - LAG(RR.RECEITAS - COALESCE(RG.GASTOS, 0)) OVER (ORDER BY RR.MES)) / NULLIF(LAG(RR.RECEITAS - COALESCE(RG.GASTOS, 0)) OVER (ORDER BY RR.MES), 0)) * 100,
			    1) AS VARIACAO_SALDO
